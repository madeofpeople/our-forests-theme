	...
	<ul class="wp-block-outermost-social-sharing is-style-logos-only">
		...
		foreach ( $services as $service ) {
			...
			<li class="outermost-social-sharing-link outermost-social-sharing-link-<?php echo \esc_attr( $service ); ?>">
				<a href="<?php echo our_forests_get_url( $service, $attributes ); ?>" data-vars-ga-category="<?php echo esc_attr( 'Share Cards' ); ?>" aria-label="<?php echo esc_attr( our_forests_get_label( $service, $attributes ) ); ?>" rel="noopener nofollow"<?php echo ( 'download' !== $service ) ? ' target="_blank"' : ''; ?> class="wp-block-outermost-social-sharing-link-anchor"<?php echo ( 'download' === $service ) ? ' download': ''; ?>>
