		...
		<div class="layout_wrapper">
			...
			<ul data-no-translation class="translation-list">
				...
				foreach ( $languages as $name => $item ) :
					$is_current = $current_url === $item['current_page_url'];
                    $classes = $item['language_name'];
                    $classes .= ( $is_current ) ? ' current' : '';
					?>
					<li class="<?php echo esc_attr( $classes ); ?>" style="list-style-image: url(<?php echo $item['flag_link']; ?>)">
						<a href="<?php echo esc_url( $item['current_page_url'] ); ?>">
							<span><?php echo esc_html( $item['language_name'] ); ?></span>
						</a>
					</li>
					<?php
				endforeach;
